...
plot(IE * (1 - a_max[1] * (1 - 0.0) * (1 - exp(-beta[1] * dose)))~dose,type="l",ylim=c(0,IE))
lines(IE * (1 - a_max[1] * (1 - 0.5) * (1 - exp(-beta[1] * dose)))~dose,col=2)
lines(IE * (1 - a_max[1] * (1 - 1) * (1 - exp(-beta[1] * dose)))~dose,col=3)
legend("bottomright",lty=1,col=1:3,legend=paste(c(0.0,0.5,1.0)),title="Phi:")
```
...
The following graphs show what happens with Kevin's parameters.

## Default epidemic

![Changing $\phi$ changes the selection for fungicide insensitivity](figures/FungResPi.png)
